...
void LCD_PrintNumber(uint8_t number) {
  // Chuy?n d?i s? thành chu?i ký t?
  char str[4];
  uint8_t temp = number;
  uint8_t digit;
  int i = 0;

  // Tru?ng h?p d?c bi?t: giá tr? s? là 0
  if (temp == 0) {
    str[i++] = '0';
  }
  else {
    // Chuy?n d?i giá tr? s? thành chu?i ngu?c
    while (temp > 0) {
      digit = temp % 10;
      str[i++] = digit + '0';
      temp /= 10;
  ...
  // Ð?o ngu?c chu?i d? d?t du?c dúng th? t?
  int j;
  char tempChar;
  for (j = 0; j < i / 2; j++) {
    tempChar = str[j];
